...
import { Paper, Box, Typography, Rating, Chip, Button } from "@mui/material";
...
const formatRecommendations = (recommendations: string) => {
  ...
  const tips = sections
    .filter(
      (s) => s.includes("tip") || s.includes("note") || s.includes("consider")
    )
    .map((s) => s.replace(/^[•\-\d.]+\s*/, "").trim())
    .slice(0, 2);
    ...
    <StyledPaper elevation={3}>
      ...
      >
        ...
        {restaurants.slice(0, 3).map((restaurant, index) => (
          <RestaurantCard key={index} variants={cardVariants}>
            <Box
              sx={{
                display: "flex",
            ...
            <Box sx={{ display: "flex", gap: 2, mt: 2 }}>
              ...
              {restaurant.opening_hours && (
                <Chip
                  ...
                  color={
                    restaurant.opening_hours.open_now ? "success" : "default"
                  }
                  size="small"
                  sx={{ borderRadius: 2 }}
                />
